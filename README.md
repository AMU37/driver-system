...
- **`SECRET_KEY`**: يجب أن تكون قيمة عشوائية 32+ حرفاً. عند `ENVIRONMENT=production` يرفض Backend الإقلاع بمفتاح ضعيف أو معروف، وعند تفعيل التكامل يُشترط `MICROSOFT_INBOUND_API_KEY` بطول 24+ حرفاً عشوائياً.
- **حسابات التجربة مقفلة في الإنتاج**: إذا استُخدمت كلمة مرور افتراضية معروفة، يُضطر المستخدم لتغييرها قبل أي عملية أخرى (تتحقق الإقامة تلقائياً عند بدء الخدمة).
- **تحديد معدل الطلبات** على تسجيل الدخول (10 محاولات فاشلة / 5 دقائق لكل مستخدم، و60 لكل عنوان IP) ونقاط التكامل الواردة (60/دقيقة لكل IP).
- لا تُستخدم حسابات التجربة في الإنتاج.
